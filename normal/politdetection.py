...
# m序列，长度为7/15
starmat,labelmat = mgenerate.generate_standard() # 星座图列表
user_pilots = mgenerate.seq_buff() # 导频列表
    ...
    for j in range(len(SNR_dB)):
        ...
        while count<repetition:
            ...
            if not (F == F_R).all():
                ...
                error = error + 1
            time_end = time.time()
            time_last = time_last + time_end - time_start

        print('本次检测 %d 个数据，共耗时%g s'%(repetition, time_last))
