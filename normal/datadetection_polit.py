...
def Pilotdetection(signal):
    ...
    for rs in range(rs_num):
        ...
        for i in range(2 ** u_num):
            # 计算欧式距离
            distance = np.sum(abs(signal_re[rs, :] - starmat[i, :]))
...
def Initialize():
    ...
    with open('上行免授权SCMA-MPA误比特率（结合导频检测）.txt', mode='a') as w:
        w.write(".................分割线................" + '\n')
        w.write('条件:数据帧长度为' + str(N_data) + '\n' + 'Eb/N0为' + str(EbN0_dB) + '\n' + '旋转因子：' + str(RF)+ '\n')
        w.write('误比特率：' + '\n')
        w.close()
    with open('上行免授权SCMA-ML误比特率（结合导频检测）.txt', mode='a') as w:
        w.write(".................分割线................" + '\n')
        w.write('条件:数据帧长度为' + str(N_data) + '\n' + 'Eb/N0为' + str(EbN0_dB) + '\n' + '旋转因子：' + str(RF)+ '\n')
        w.write('误比特率：' + '\n')
        w.close()
    with open('上行免授权SCMA-MPA丢包率（结合导频检测）.txt', mode='a') as w:
        w.write(".................分割线................" + '\n')
        w.write('条件:数据帧长度为' + str(N_data) + '\n' + 'Eb/N0为' + str(EbN0_dB) + '\n' + '旋转因子：' + str(RF)+ '\n')
        w.write('丢包率：' + '\n')
        w.close()
    with open('上行免授权SCMA-ML丢包率（结合导频检测）.txt', mode='a') as w:
        w.write(".................分割线................" + '\n')
        w.write('条件:数据帧长度为' + str(N_data) + '\n' + 'Eb/N0为' + str(EbN0_dB) + '\n' + '旋转因子：' + str(RF)+ '\n')
        w.write('丢包率：' + '\n')
        w.close()
